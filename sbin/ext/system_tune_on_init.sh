...
[ "`$BB grep -i ro.build.PDA=I777XXLSJ /system/build.prop`" ] && JBSAMMY=1;
[ "`$BB grep -i ro.build.PDA=I777XWLS8 /system/build.prop`" ] && JBSAMMY=1;
...
if [ -e /system/bin/fsck_msdos ]; then
	...
	$BB echo "CANT FIX SDCARDS, REPORT TO AJ" > $LOG_SDCARDS;
fi;
